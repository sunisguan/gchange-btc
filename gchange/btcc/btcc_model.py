# -*- coding: utf-8 -*-
from btcc_model_db import HistoryDataForDb
import common
import datetime
...
class HistoryData(object):
    ...
    def to_historydata(self):
        if HistoryDataForDb.select().where(HistoryDataForDb.tid == self.get_tid()).exists():
            return HistoryDataForDb.get(HistoryDataForDb.tid == self.get_tid())
        else:
            return HistoryDataForDb.create(tid=self.get_tid(), timestamp=self.get_date(), amount=self.get_amount(), type=self.get_type(), price=self.get_price())
