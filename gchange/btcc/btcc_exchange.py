...
class BtccExchange(object):
    ...
    def buy(self, amount, price=None, market=bhc.BtccHttpClient.MarketParams.BTC_CNY):
        ...
        return bm.Order(id=resp, status=common.OrderStatus.OPEN)
        """
        if status:
            ...
            if status:
                ...
                return bm.Order(id=order_id, status=common.OrderStatus.OPEN)
        else:
...
class BtccWebsocketClient(BaseNamespace):
    ...
    def on_grouporder(self, *args):
        # 接收到市场深度
        self.__dispatch(self.Event.ON_MARKETDEPTH, None)

    def on_order(self, *args):
        # 接收到订单状态更新
        common.logger.info("exchange 收到挂单状态更新")
        self.__dispatch(self.Event.ON_ORDER_BOOK_UPDATE, bm.Order(**args[0]))

    def on_account_info(self, *args):
        common.logger.info("exchange ON_ACCOUNT_INFO")
...
class WebSocketClientThread(threading.Thread):
    ...
    def start(self):
        self.__socketIO = SocketIO('sio.btcc.com', 80)
